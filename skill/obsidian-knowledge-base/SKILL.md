...
当用户通过 `/obsidian-knowledge-base` 触发本 skill 时，**必须先检测当前目录是不是一个已初始化的 Vault**（是否有 `笔记/` 或 `raw/` 目录）。
...
检测到当前目录还没有初始化，想现在搭建吗？

🚀 1. 初始化知识库（推荐）
     自动：安装 Obsidian → 建目录 → 拉取文件 → 安装子 Skill
📖  2. 查看帮助
```

用户选择 1 → 进入下方的 Init 流程。
